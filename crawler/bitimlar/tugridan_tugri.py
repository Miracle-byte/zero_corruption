# http://xarid.uz/ajax/dxarid?page=3&auction=Adver&status=Completed&year=2021
import requests
...
for i in range(1,200):
    url = "http://xarid.uz/ajax/dxarid?page=%d&auction=Adver&status=Completed&year=2021" % (i)
    print(url)
    r = requests.get(url)
    print(r.status_code)
    #print(r.content)
    ...
    for row in rows:
        ...
        start_narx = cols[2].text.strip().replace("UZS","").replace(" ","").split(',')[0]
        ...
        zakazchik = cols[4].text.strip()
        zakazchik_inn = cols[5].text.strip()
        print(nom,start_narx,kontrakt_narx, zakazchik,zakazchik_inn)
        #continue
