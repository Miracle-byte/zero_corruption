# http://xarid.uz/ajax/milliy?page=570&status=Completed&year=2021
import requests
...
for i in range(1,600):
    url = "http://xarid.uz/ajax/milliy?page=%d&status=Completed&year=2021" % (i)
    print(url)
    r = requests.get(url)
    print(r.status_code)
    #print(r.content)
    ...
    for row in rows:
        ...
        zakazchik = cols[12].text.strip()
        zakazchik_inn = cols[11].text.strip()
