# http://xarid.uz/ajax/dxarid?page=2&auction=BestOffer&status=Completed&year=2021
import requests
...
for i in range(1,100):
    url = "http://xarid.uz/ajax/dxarid?page=%d&auction=BestOffer&status=Completed&year=2021" % (i)
    print(url)
    r = requests.get(url)
    print(r.status_code)
    #print(r.content)
    ...
    for row in rows:
        ...
        count = 1
        nom = cols[4].text.strip()
        start_narx = cols[5].text.strip().replace("UZS","").replace(" ","").split(',')[0]
        kontrakt_narx = cols[6].text.strip().replace("UZS","").replace(" ","").split(',')[0]
        zakazchik = cols[7].text.strip()
        zakazchik_inn = cols[8].text.strip()
